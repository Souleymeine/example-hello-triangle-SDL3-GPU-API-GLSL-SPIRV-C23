...
int main()
{
	...
	s_vertbuf = SDL_CreateGPUBuffer(s_gpudev, &(struct SDL_GPUBufferCreateInfo) {
		.size = sizeof(tri_verts),
		.usage = SDL_GPU_BUFFERUSAGE_VERTEX,
	});
	s_transferbuf = SDL_CreateGPUTransferBuffer(s_gpudev, &(struct SDL_GPUTransferBufferCreateInfo) {
		.size = sizeof(tri_verts),
	...
	SDL_GPURenderPass* render_pass = SDL_BeginGPURenderPass(cmdbuf, &(struct SDL_GPUColorTargetInfo) {
	...
	SDL_BindGPUVertexBuffers(render_pass, 0, (struct SDL_GPUBufferBinding[1]) {{
...
//	--- Main Loop ---
	SDL_Event event;
	while (running) {
		if (SDL_WaitEvent(&event) && (event.type == SDL_EVENT_QUIT || event.key.key == SDLK_ESCAPE)) {
			running = false;
		}
	}
//	End Main Loop ---
